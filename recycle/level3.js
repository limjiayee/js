...
this.bg.setOrigin(0, 0);
    ...
    this.apple = this.physics.add.sprite(524, 644, 'apple');
    this.banana = this.physics.add.sprite(800, 600, 'banana');
    this.bone = this.physics.add.sprite(1000, 626, 'bone');
    //this.apple = this.physics.add.sprite(951, 644, 'apple');
    this.red = this.physics.add.sprite(1400, 950, 'red');
    ...
    this.yellowText = this.add.text(20, 40, 'Kitchen waste 0', {
        fontSize: '30px',
        fill: '#ffffff'
    });
    ...
    this.add.text(1500,40, 'Level 3', { font: '30px Courier', fill: '#ffffff' });
...
//   this.cameras.main.setBackgroundColor('#ccccff');
    ...
    this.physics.add.overlap(this.player, this.apple,this.holdapple, null, this );
    this.physics.add.overlap(this.player, this.banana,this.holdbanana, null, this );
    this.physics.add.overlap(this.player, this.bone,this.holdbone, null, this ); 
...
//apple
holdapple(player) {
    this.apple.x = this.player.x+64
    this.apple.y = this.player.y
    this.collectapple=1
    return false;
}
 ...
 //banana
 holdbanana(player) {
    this.banana.x = this.player.x+64
    this.banana.y = this.player.y
    this.collectbanana=1
    return false;
}

//bone
holdbone(player) {
    this.bone.x = this.player.x+64
    this.bone.y = this.player.y
    this.collectbone=1
    return false;
}

dropgrey(player) {
   if (this.collectapple) {
       console.log('dropapple')
   ...
   } else if (this.collectbanana) {
    console.log('dropbanana')
   ...
   } else if (this.collectbone) {
        console.log('dropbone')
        this.bone.disableBody(true, true);
...
update() {
 ...
 // use the original sprite looking to the right
    ...
    if ( this.paperCount > 3 ) {
        console.log('Collected 1 star, jump to level 4');
