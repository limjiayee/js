...
preload() {
    ...
    //this.load.atlas('boy', 'assets/cleaner.png', 'assets/cleaner.json');
    this.load.image('toiletpaper', 'assets/toiletpaper.png');
    this.load.image('tv', 'assets/tv.png');
    this.load.image('takeaway', 'assets/takeaway.png');
    this.load.image('red', 'assets/red_dusbin_v02.png');
    this.load.image('green', 'assets/green_dusbin_v02b.png');
...
this.bg.setOrigin(0, 0);
    ...
    this.takeaway = this.physics.add.sprite(1100, 600, 'takeaway');
    this.tv = this.physics.add.sprite(1351, 626, 'tv');
    ...
    this.yellowText = this.add.text(20, 40, 'Other waste 0', {
        fontSize: '30px',
        fill: '#ffffff'
    });
    ...
    this.add.text(1500,40, 'Level 1', { font: '30px Courier', fill: '#ffffff' });
...
//   this.cameras.main.setBackgroundColor('#ccccff');
    ...
    // this.physics.add.collider(this.platformLayer, this.takeaway);
    // this.physics.add.collider(this.platformLayer, this.tv);

    //this.physics.add.collider(this.groundLayer, this.toiletpaper, this.takeaway, this.tv);
    this.physics.add.collider(this.blackwhiteLayer, this.player);


    // this.physics.add.collider(this.platformLayer, this.takeaway);
    // this.physics.add.collider(this.groundLayer, this.takeaway);
    // this.physics.add.collider(this.blackwhiteLayer, this.player);


    this.physics.add.overlap(this.player, this.toiletpaper,this.holdtoiletpaper, null, this );
    this.physics.add.overlap(this.player, this.takeaway,this.holdtakeaway, null, this );
    this.physics.add.overlap(this.player, this.tv,this.holdtv, null, this );
...
holdtoiletpaper(player) {
 ...
 //takeaway
 holdtakeaway(player) {
    this.takeaway.x = this.player.x+64
    this.takeaway.y = this.player.y
    this.collecttakeaway=1
    return false;
}

//tv
holdtv(player) {
    this.tv.x = this.player.x+64
    this.tv.y = this.player.y
    this.collecttv=1
    return false;
}
...
dropyellow(player) {
   ...
   } else if (this.collecttakeaway) {
    console.log('droptakeaway')
    this.takeaway.disableBody(true, true);
   ...
   } else if (this.collecttv) {
        console.log('droptv')
        this.tv.disableBody(true, true);
